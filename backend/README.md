...
1. Clone the repository:
    ...
    ```bash
    git clone https://github.com/MingPV/Polltato.git
    cd Polltato
    ```
...
3. Create and configure the environment file:
    ...
    Open the `.env` file and fill in all required configuration values:
    ...
    - **Browser clients:** set `CORS_ORIGIN` to your frontend origin (e.g. `http://localhost:3000`). The API uses an HttpOnly cookie (`access_token`) for JWT sessions; credentialed CORS requires an explicit origin (not `*`).

    **Note:** This project uses a single `.env` file for both development and testing environments.

4. Start PostgreSQL services using Docker Compose (reads `.env` in `backend/` automatically):
...
5. Run the application:
    ...
    The application will start on port `8000` by default (configurable via `APP_PORT` in `.env`).
...
6. Run tests:
    ...
    **Note:** Tests use the same `.env` file. Make sure to configure test database credentials (`DB_TEST_*`) in your `.env` file.
...
The project uses a single `.env` file for configuration. Key environment variables include:
...
The test suite uses the same `.env` file and automatically:
...
**Important:** Make sure to configure `DB_TEST_NAME`, `DB_TEST_USER`, `DB_TEST_PASSWORD`, and `DB_TEST_PORT` in your `.env` file for tests to work properly.
